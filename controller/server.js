...
app.post('/login' ,(req, res) => {
    ...
    if (isLoggedIn) {
        ...
        if (isAdmin) {
            if(loginAsAdmin === "on") {
                res.sendFile(path.join(__dirname, '../public', 'adminMainWindow.html'))
            }
            else {
...
app.get('/infections', (req, res) => {
    res.sendFile(path.join(__dirname, '../public', 'infections.html'))
})
...
app.get('/vaccinations', (req, res) => {
    res.sendFile(path.join(__dirname, '../public', 'vaccinations.html'))
})
...
app.post('/getTotalDeaths', (req,res) => {
    ...
    if (totalDeath === false) {
        ...
        let result = writeInHtml("Total deaths in "+country+" so far is: " +totalDeath[0][0]+"\n")
        res.write(result)
        res.end()
    }
})
...
app.post('/getTodayDeaths', (req,res) => {
    ...
    if (totalDeath === false) {
        ...
        let result = writeInHtml("Total deaths in "+country+" today is: " +totalDeath[0][0]+"\n")
        res.write(result)
        res.end()
    }
})
...
app.post('/getAverage', (req,res) => {
    ...
    if (totalDeath === false) {
        ...
        let result = writeInHtml("Average daily deaths in "+country+" is: " +totalDeath[0][1]+"\n")
        res.write(result)
        res.end()
    }
})
...
app.post('/getTotalInfected', (req,res) => {
    let country = req.body.Country1
    let totalInfected = doQueries.sumDailyCasesQueryByLocation(country).total_daily_cases
    if (totalInfected === undefined) {
        ...
        let result = writeInHtml("Total COVID infected in "+country+" is: " +totalInfected+"\n")
        res.write(result)
        res.end()
    }
})
...
app.post('/getInfectedPerVariant', (req,res) => {
    ...
    let totalInfected = doQueries.sumDailyCasesQueryByLocationAndVariant(country, variantKey)
    if (totalInfected === false) {
        ...
        let result = writeInHtml("Total COVID infected in "+country+" from "+variantName+ " is: " +totalInfected[0][0]+"\n")
        res.write(result)
        res.end()
    }
})
...
app.post('/getAverageCases', (req,res) => {
    ...
    if (avgCases === false) {
        ...
        let result = writeInHtml("The average daily infection cases in "+country+" is: " +avgCases[0][1]+"\n")
        res.write(result)
        res.end()
    }
})
...
app.post('/getTotalVaccinated', (req,res) => {
    ...
    if (totalVaccinated === false) {
        ...
        let result = writeInHtml("Total vaccinated people in "+country+" is: " +totalVaccinated[0][0]+"\n")
        res.write(result)
        res.end()
    }
})
...
app.post('/getVaccinatedPerManufacturer', (req,res) => {
    ...
    let manufacturerName = getKeyByValue(manufacturerMap, parseInt(manufacturerKey))
    let vaccinations = doQueries.getTotalVaccinationsByManufacturer(country, manufacturerKey)
    if (vaccinations === false) {
        ...
        let result = writeInHtml("Total vaccinated in "+country+" in "+manufacturerName+ " vaccine is: " +vaccinations[0].total_vaccinations+"\n")
        res.write(result)
        res.end()
    }
})
...
app.post('/updateInfectionCases',(req, res)=>{
    ...
    let result = doQueries.updateInfectionCasesQuery(location, date, variant, daily_cases)
    let text = writeInHtml(result)
    res.write(text)
    res.end()
})
...
function writeInHtml(text){
    result = `<p style="font-size:30px; font-family:'verdana';">`+text+`</p>`
    return result
}